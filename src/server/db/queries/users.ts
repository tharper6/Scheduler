...
const findOneById = (id: number) => Query(`SELECT users.id, users.username, users.email, users.role, users.trainingrole, users.bio, users.avatar, sports.sportname FROM users JOIN sports ON users.sportid = sports.id WHERE users.id = ?`, [id]);
