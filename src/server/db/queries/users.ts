...
const getAll = () => Query('SELECT id, username, sportid, role FROM users');
...
const updateAvatar = (avatar: string, id: number) => Query('UPDATE users SET avatar = ? WHERE id = ?', [avatar, id])
